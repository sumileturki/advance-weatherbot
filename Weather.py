...
class Weather(Tk):
    def __init__(self):
        ...
        self.iconbitmap("weather_icon.ico")
        self.resizable(False,False)
        self.__gui()
        threading.Thread(target=self.__gui).start()
        # mixer.music.load("Har-Funn-Maula(PaglaSongs).mp3")
        # mixer.music.play(-1)
       
        
    def __gui(self):
       # placing the black border for search
       self.img=Image.open("black_border.png")
       ...
       self.img1=Image.open("search_btn.png")
       ...
       # location image logo
       self.img2=Image.open('location.png')
       ...
       # sunrise logo
       self.finalimg4=ImageTk.PhotoImage(image=Image.open("sunrise.png").resize((40,40)))
       ...
       #sunset logo
       self.finalimg5=ImageTk.PhotoImage(image=Image.open("sunset.png").resize((40,30)))
       ...
       # bottom bar
       self.finalimg6=ImageTk.PhotoImage(image=Image.open('bottom_bar.png').resize((770,70)))
       ...
       self.email_label = Label(text="Enter Email:", font='Arial 14 bold')
       self.email_label.place(x=15, y=430)

       self.email_entry = Entry(font=("Segoe UI", 14, 'bold'), width=22, justify="center", relief="flat")
       self.email_entry.place(x=150, y=430)
       self.img1=Image.open("mail.png")
       self.resizeim1=self.img1.resize((35,33))
       self.emailbuu=ImageTk.PhotoImage(self.resizeim1)
       self.send_email_button = Button(image=self.emailbuu,bg="black",command=self.send_email)
       self.send_email_button.place(x=400, y=425)
    ...
    def __get_weather(self):
        try:
            ...
            if weather_response.status_code == 200:
                ...
                self.__set_information(weather=weather_data)

                # Display notification
                self.show_notification("Weather Update", f"Weather information for {city} retrieved successfully.")

            elif weather_response.status_code == 404:
                messagebox.showerror("Error", "City Not Found")
                self.search.set("")
            else:
                messagebox.showerror("Error", f"Error {weather_response.status_code}: Something went wrong")
    ...
    def place_image(self, type):
        if type == "Clear":
            img = "clear.png"
            music_file = "birds-singing-calm-river-nature-ambient-sound-127411.mp3"
        elif type == "Clouds":
            img = 'clouds.png'
            music_file = "natural-thunder-113219.mp3"
        elif type == "Rain":
            img = 'rain.png'
            music_file = "Rain-On-Umbrella-A1-www.fesliyanstudios.com.mp3"
        elif type == 'Haze':
            img = 'haze.png'
            music_file = "haze_music.mp3"
        else:
            img = 'main.png'
            music_file = "aspirants-theme-bgm-tvf.mp3"
    ...
    def send_email(self):
        to_address = self.email_entry.get()
        if not to_address:
        ...
        description = self.des.cget("text")
        temperature = self.temperature.cget("text")

        # Compose email message
        message = f"Hello bennettians this is from Ctrl Alt Del team \n \nWeather in {city}: {description}, Temperature: {temperature}°C. "
